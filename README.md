...
8. **Dock** — Pins apps listed in `config/dock.yml` to the Dock via `dockutil` (idempotent: skips apps already pinned via `dockutil --find`). Restarts the Dock once after all adds.
