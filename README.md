...
8. **macOS Defaults** — Applies system preferences from `config/macos_defaults.yml` (Finder, Dock, Software Update, etc.). Every Software Update auto-* knob is locked off (`AutomaticCheckEnabled`, `AutomaticDownload`, `AutomaticallyInstallMacOSUpdates`, `CriticalUpdateInstall`, `ConfigDataInstall`) so a remote Mac never silently stages an OS update that bricks SSH on next reboot — relevant for headless servers where you can't recover from "preparing update" remotely.
