...
| `dotfiles.yml` | YAML: `repo` (git URL) | Dotfiles module — clones the repo into `~/dotfiles` (or ff-pulls if present) before ClaudeCode runs. Hand-authored — no live source on the Mac to harvest from. Harvester scaffolds an empty template with `repo: REPLACE_ME`; fill in or delete the file to skip the module. |
